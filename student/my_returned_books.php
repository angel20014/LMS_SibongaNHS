...
$returned_query = mysqli_query($con, "
    SELECT 
        ...
        (SELECT rb.return_status 
         FROM return_book rb 
         WHERE rb.book_id = bb.book_id AND rb.student_id = '$student_id'
    ...
    WHERE bb.student_id = '$student_id' AND bb.borrowed_status = 'returned'
    GROUP BY bb.book_id
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            while ($row = mysqli_fetch_assoc($returned_query)) {
                                ...
                                $date_borrowed = date('M d, Y H:i:s', strtotime($row['date_borrowed']));
                                $due_date = date('M d, Y', strtotime($row['due_date']));
                                $date_returned = $row['date_returned'] ? date('M d, Y H:i:s', strtotime($row['date_returned'])) : '-';
