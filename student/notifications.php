...
// Fetch borrow, return, profile registration, and login/logout events
$query = "
SELECT 
    b.book_title,
    br.date_borrowed,
...
SELECT 'Profile Registered' AS book_title,
       s.date_registered AS date_borrowed,
       NULL AS due_date,
...
SELECT 'Login Event' AS book_title,
       slh.login_time AS date_borrowed,
       NULL AS due_date,
       NULL AS date_returned,
       NULL AS return_status,
       'login' AS borrowed_status,
       NULL AS admin_firstname,
       NULL AS admin_lastname
...
SELECT 'Logout Event' AS book_title,
       slh.logout_time AS date_borrowed,
       NULL AS due_date,
       NULL AS date_returned,
       NULL AS return_status,
       'logout' AS borrowed_status,
       NULL AS admin_firstname,
       NULL AS admin_lastname
FROM student_login_history slh
WHERE slh.student_id = '$student_id'
    ...
    while ($row = mysqli_fetch_assoc($result)):
        ...
        switch ($status) {
            ...
            case 'profile_registered':
                $message = "You registered your student profile.";
                $icon = 'fa-user-plus';
                $time_display = date("M d, Y h:i:s a", strtotime($row['date_borrowed']));
                break;
            case 'login':
            ...
            case 'logout':
                ...
                break;
            default:
                $message = "Your return request for \"$title\"is still pending..";
                $icon = 'fa-info-circle';
                $time_display = $now;
        }
