...
$teacher_id = $_SESSION['id']; // Changed from student_id to teacher_id
$today = date('Y-m-d');
...
$due_query = mysqli_query($con, "
    ...
    WHERE br.teacher_id = '$teacher_id'  -- Changed from student_id
...
while ($row = mysqli_fetch_assoc($due_query)) {
    ...
    if ($dueDate >= $todayTime) {
        ...
        if ($daysLeft == 0) {
            $notif_messages[] = [
                "icon" => "",
                "msg"  => "You have <b>TODAY</b> to return <b>" . htmlspecialchars($row['book_title']) . "</b> (Due date is on " . date("M d, Y", $dueDate) . ")."
            ];
        } elseif ($daysLeft > 0) {
            $notif_messages[] = [
                "icon" => "",
                "msg"  => "You have <b>$daysLeft day(s)</b> left before the due date of <b>" . htmlspecialchars($row['book_title']) . "</b> (Due date is on " . date("M d, Y", $dueDate) . ")."
            ];
        }
    }
}
...
<div class="tile_row">
    ...
    <!-- Books Borrowed by Teacher -->
    <div class="tile_stats_count tile-container">
        <?php 
        $borrow_result = mysqli_query($con, "SELECT * FROM borrow_book WHERE teacher_id='$teacher_id'");
        ?>
    ...
    <div class="tile_stats_count tile-container">
        <?php 
        $pending_return_result = mysqli_query($con, "SELECT * FROM return_book WHERE teacher_id='$teacher_id' AND return_status='pending'");
        ?>
...
<div class="row">
    ...
    <!-- Recently Borrowed Books by Teacher -->
    <div class="col-md-4">
    ...
    <!-- Recently Returned Books by Teacher -->
    <div class="col-md-4">
        <div class="recent-table">
